...
const [...keys] = users3;
let persons1skill = [];
for (var { skills, name, ...rest } of keys) {
  if (skills.length < 2) {
    persons1skill.push(name);
  }
}
console.log(persons1skill);
...
let [...countries3] = countries_data;

for (var { name, capital, population, languages, ...rest } of countries3) {
...
var newStudent = [...ArraytoObject(students)];

const transformedStudents = newStudent.map(function ({ skills, score, ...rest }) {
  let [htmlScore, cssScore, jsScore, reactScore] = score;
  let [html, css, js, react] = skills;

  const obj = [
    { skill: html, score: htmlScore },
    { skill: css, score: cssScore },
    { skill: js, score: jsScore },
    { skill: react, score: reactScore },
  ];
  return { skills: obj, score, ...rest };
});
