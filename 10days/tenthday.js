...
let newarray = [...SetC];
const stringElement = newarray.map(String);
const Stringset = new Set(stringElement);
console.log(Stringset);

const countriescharcountmap = new Map();
countries.forEach(function (country) {
  const charCount = country.length;
  countriescharcountmap.set(country, charCount);
});
console.log(countriescharcountmap);

const A = new Set(a);
const B = new Set(b);
const UnionAB = new Set(c);
console.log(UnionAB);
const intersectionAB = a.filter((numb) => B.has(numb));
console.log(intersectionAB);
const awithb = b.filter((numb) => !A.has(numb));
console.log(awithb);
...
var mostspeakinglanguage = function (countries, n) {
  var newarray1 = [];

  var languagemap = countries.map((val) => val.languages);
  languagemap.forEach(function (value) {
    value.forEach((language) => newarray1.push(language));
  });
  let setnewarray1 = new Set(newarray1);
  console.log(`Có tổng cộng ${setnewarray1.size} ngôn ngữ`);
  const counts = [];
  const count = {};
  for (const l of setnewarray1) {
    const filterLang = newarray1.filter((lng) => lng === l);
    counts.push({ lang: l, count: filterLang.length });
  }
  var mostspeakinglanguage = counts.sort((a, b) => b.count - a.count).slice(0, n);
  return mostspeakinglanguage;
};
console.log(mostspeakinglanguage(countries_data, 10));
